...
validate.single.numeric <- function(number, var.name) {
  ...
  if (!is.numeric(number)) stop(paste(var.name, " must be numeric!", sep = ""))
...
fbroc.plot.add.metric.paired <- function(x, roc.plot,show.metric, show.area, perf, 
  ...
  if (show.metric == "tpr") {
    ...
    roc.plot <- roc.plot + geom_errorbar(data = extra.frame, width = 0.02, linewidth = 1.25,
  ...
  if (show.metric == "fpr") {
    extra.frame <- data.frame(TPR = perf$params, 
                              ...
                              upper = c(perf$CI.Performance.Predictor1[2],
                                        perf$CI.Performance.Predictor2[2]))
    roc.plot <- roc.plot + geom_errorbarh(data = extra.frame, height = 0.02, linewidth = 1.25,
